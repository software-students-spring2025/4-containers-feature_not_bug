...
def app_setup():
    ...
    dbname = os.getenv("MONGO_DB", "dutch_pay")  # Ensure it loads correctly from the .env file
    ...
    def upload():
        data = []
        if "capture-receipt" not in request.form and "upload-receipt" not in request.form:
        ...
        if "person-" + str(num) + "-name" not in request.form or "person-" + str(num + 1) + "-name" in request.form:
        ...
        for i in range(0, num):
            data.append(("person-" + str(i + 1) + "-name", request.form["person-" + str(i + 1) + "-name"]))
            data.append(("person-" + str(i + 1) + "-items", request.form["person-" + str(i + 1) + "-desc"]))
        ...
        try:
            ...
            return f"Error processing receipt: {res.text}", 400
        except requests.RequestException as req_error:
            error_msg = f"Error connecting to ML client: {str(req_error)}"
